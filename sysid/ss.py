...
class StateSpaceDiscreteLinear(object):
    ...
    def __init__(self, A, B, C, D, Q, R, dt):
        ...
        n_x = self.A.shape[0] # number of states
        n_u = self.B.shape[1] # number of inputs
        n_y = self.C.shape[0] # number of outputs
        
        #print("n_x:{}, n_u:{}, n_y:{}".format(n_x,n_u,n_y))
        ...
        assert self.R.shape[0] == n_y # or is it n_u?
        assert self.R.shape[1] == n_y # or is it n_u?
    ...
    def simulate(self, f_u, x0, tf):
        ...
        assert np.matrix(f_u(0, x0, 0)).shape[0] == n_u # fix, come on man

        # take square root of noise cov to prepare for noise sim
        if scipy.linalg.norm(self.Q) > 0:
            sqrtQ = scipy.linalg.sqrtm(self.Q)
        else:
            sqrtQ = self.Q

        if scipy.linalg.norm(self.R) > 0:
